...
import { publishEvent } from "@/services/event-domain";
    ...
    if (!parsed.success) {
      ...
      await writeAdminAuditLogBestEffort({
        ...
        details: {
          ...
          validationError: firstIssue?.message ?? "Invalid publish payload.",
        },
      });
      return failure(firstIssue?.message ?? "Invalid publish payload.", 400);
    }

    try {
      const event = await publishEvent(parsed.data.eventId);
      await writeAdminAuditLogBestEffort({
        ...actor,
        action: "admin.event.publish",
        resourceType: "event",
